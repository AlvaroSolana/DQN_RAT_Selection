...
class State(ProtoState):
    ...
    def to_sep_numpy(self, idx, nm, ns): 
        """
        This function normalizes the values of the state and returns the state as a pytorch tensor
        ...
        if len(self.q) > 1:
            ...
            return torch.tensor([ (self.t - nm[0])/ns[0] , 
                                  ...
                                  ((torch.sum(dq) - dq[idx])/(len(dq)-1) - nm[4])/ns[4],
                                  # Last line wasn´t there, replicated to make it work 
            ...
            return torch.tensor([ (self.t - nm[0])/ns[0] , 
                                  ...
                                  (self.i - nm[3])/ns[3],
                                  (self.i - nm[3])/ns[3],
                                  (self.i - nm[3])/ns[3],]).float().to(device) # ''' I added two lines'''


        
    def to_combine_tens(self, nm, ns): # not used in training
    ...
    def to_sep_tensor(self, idx, **kwargs):  # not used in training
        non_inv, inv = self.to_sep_numpy(idx)
        return torch.tensor(non_inv, **kwargs), torch.tensor(inv, **kwargs)
    
    def to_sep_tensor_less(self, idx, nm, ns, mean=False, q0=True): ## NOT USED in training
        """
        :param nm & ns: (t, p, q, i) normalized mean and std
        """
        if q0:
        ...
        if mean:
            if len(self.q) > 1:
                ...
                return torch.stack([(self.t - nm[0])/ns[0], 
                                    (self.p - self.i - nm[1])/ns[1], 
                                    (self.q[idx] - nm[2])/ns[2], 
                                    (self.i - nm[3])/ns[3]]).float().to(device)
        else:
            return None
...
#        self.r = lambda t, Q, S, dS, nu: - nu * (S + self.t_cost * nu) * self.dt + (self.t >= self.T) * ( (S+dS) * Q - self.L_cost * Q**2)
        ...
        self.dS = torch.tensor(0.0).to(device)
        self.dF = torch.tensor(0.0).to(device)
        self.dI = torch.tensor(0.0).to(device)
        
        self.total_reward = torch.tensor(0.0).to(device)
        self.Q0 = torch.tensor(0.0).to(device)
    ...
    def r(self, t, Q, S, dS, nu, to_print=False):
        ...
        #return - nu * (S + self.t_cost * nu) * self.dt + ( -(Q-nu*self.dt)*S + Q*(S+dS) ) + (t >= self.T) * ( - self.L_cost * Q**2) - self.phi * self.dt * Q**2
        return  ( -(Q)*S + Q*(S+dS) ) + (t >= self.T) * ( - self.L_cost * Q**2) - self.phi * self.dt * Q**2
    ...
    def reset(self):
        ...
        """
        self.Q = (torch.randn(self.N).to(device) * self.sigma_Q0)
        self.Q0 = self.Q.clone()
        self.S = torch.squeeze((10 + torch.randn(1).to(device) * self.sigma0))
        
        self.I = torch.squeeze((torch.randn(1).to(device) * 0.5 * self.sigma))
        self.t = torch.tensor(0.0).to(device)

        self.last_reward = torch.zeros(self.N).to(device)
        self.total_reward = torch.zeros(self.N).to(device)

        temp = (torch.ceil(self.T / self.dt) + 2).detach().cpu().numpy()
        self.dW = (torch.randn(int(np.round(temp))).to(device) * torch.sqrt(self.dt))

        self.done = False

    def setInv(self, inv): # WE DON´T NEED THIS ONE NOW
        """
    ...
    def step(self, nu, to_print=False):
        ...
        if not self.done:
            ...
            # if nu*self.dt.shape==[]: resize or create another vector or if it´s none just create a new one with zeros maybe
            
            #EEROR HERE BECAUSE OF DIFFERENT SIZES
            if self.Q.shape==self.dt.shape:
                self.Q = self.Q.clone() + nu*self.dt
                self.t = self.t.clone() + self.dt
                print("size error corrected")
            ...
            # CHANGE
            self.last_reward = self.r(self.t.clone(), self.Q.clone(), self.S.clone(), self.dS.clone(), nu, to_print)[:self.N]
